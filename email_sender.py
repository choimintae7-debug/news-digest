"""email_sender.py — Gmail SMTP를 통한 HTML 뉴스레터 발송"""
import smtplib
import datetime
...
def _build_html(name: str, articles_by_topic: dict) -> str:
    ...
    for topic, articles in articles_by_topic.items():
        ...
        <div style="margin-bottom:32px;">
          <h2 style="font-size:18px;font-weight:700;color:#4F8EF7;border-bottom:2px solid #e8edf5;padding-bottom:8px;margin-bottom:14px;">
            🔍 {topic}
          </h2>
          {items_html}
        </div>"""
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#eef2f7;padding:32px 0;">
    <tr><td align="center">
      <table width="620" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#4F8EF7 0%,#8A56F7 100%);padding:32px 40px;">
            <h1 style="margin:0;color:#fff;font-size:26px;font-weight:700;">📬 뉴스 다이제스트</h1>
            <p style="margin:8px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">{today} | {name}님을 위한 맞춤 뉴스</p>
          </td>
        </tr>

        <tr>
          <td style="padding:12px 40px;background:#f4f6fb;border-bottom:1px solid #e8edf5;">
            <span style="font-size:12px;color:#666;">출처:&nbsp;
              <span style="font-size:10px;font-weight:700;background:#03C75A;color:#fff;border-radius:4px;padding:1px 5px;">N 네이버</span>
              &nbsp;네이버 뉴스&nbsp;&nbsp;
              <span style="font-size:10px;font-weight:700;background:#4F8EF7;color:#fff;border-radius:4px;padding:1px 5px;">G 글로벌</span>
              &nbsp;NewsAPI
            </span>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:32px 40px;">{topic_blocks}</td>
        </tr>

        <tr>
          <td style="background:#f4f6fb;padding:20px 40px;text-align:center;border-top:1px solid #e8edf5;">
            <p style="margin:0;font-size:12px;color:#aaa;">
              본 메일은 자동 발송됩니다 &nbsp;·&nbsp; 구독 취소는 관리자에게 문의하세요
            </p>
          </td>
        </tr>
